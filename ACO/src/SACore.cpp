...
void SACore::SA() {
	...
	std::chrono::high_resolution_clock::time_point sBeginTime = std::chrono::high_resolution_clock::now();
	...
	do{
		...
		std::chrono::duration<double> m_lfRunTime = sEndTime - sBeginTime;
		if(m_lfRunTime.count() >= m_wriResultsWriter.CurrSamplingTime()){
