...
#define DEFAULT_INPUT_PATH "data/input_info/input.txt"
...
int main (int argc, char **argv)
{
	...
	std::cout << std::endl;
	FILE* f;
	f = fopen(DEFAULT_INPUT_PATH,"r");
	fscanf(f, "INPUT_FILE_NAME %s\n", inputFileName);
	fscanf(f, "seedsFILENAME %s\n", seedsFileName);
	fscanf(f, "ALPHA %lf\n", &alpha);
	fscanf(f, "BETA %lf\n", &beta);
	fscanf(f, "RHO %lf\n", &rho);
	fscanf(f, "EPSILON %lf\n", &epsilon);
	fscanf(f, "ANTS %d\n", &ants);
	fscanf(f, "RUNS %d\n", &runs);
	fscanf(f, "BEST_KNOWN_SOLUTION %d\n", &bestKnownSolution);
	fscanf(f, "MAX_RUNTIME %lf\n", &max_runtime);
	if (alpha==-1) alpha = DEFAULT_ALPHA;
	if (beta==-1) beta = DEFAULT_BETA;
	if (rho==-1) rho = DEFAULT_RHO;
	if (epsilon==-1) epsilon = DEFAULT_EPSILON;
	if (ants==-1) ants = DEFAULT_ANTS;
	if (runs==-1) runs = DEFAULT_RUNS;
	if (bestKnownSolution==-1) bestKnownSolution = DEFAULT_BEST_KNOWN_SOLUTION;
	if (max_runtime==-1) max_runtime = DEFAULT_MAX_RUNTIME;
	//if(argc > 1){
	//	while (true)
	//	{
